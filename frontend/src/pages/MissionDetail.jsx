...
function FeeNotice({ totalAmount }) {
  ...
  return (
    <div style={{ background: '#ef444411', border: '1px solid #ef444433', borderRadius: 'var(--radius-sm)', padding: '12px 14px', marginBottom: 12 }}>
      <div style={{ fontWeight: 700, fontSize: 13, color: '#ef4444', marginBottom: 6 }}>⚠️ 만료 전 수수료 납부 필수</div>
      ...
      <a href={PLATFORM_OPENCHAT} target="_blank" rel="noopener noreferrer"
        style={{ display: 'inline-block', marginTop: 8, background: '#FEE500', color: '#000000CC', borderRadius: 8, padding: '7px 14px', fontSize: 12, fontWeight: 700, textDecoration: 'none' }}>
        💬 수수료 납부하러 가기
      </a>
    </div>
  )
...
function PledgeForm({ missionId, refetch }) {
  ...
  const handlePledge = async () => {
    ...
    setLoading(true)
    try {
      await createPledge({ mission_id: missionId, amount: parsed })
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
        {QUICK.map(q => (
          <button key={q} onClick={() => setAmount(q.toLocaleString())}
            style={{ padding: '7px 12px', borderRadius: 99, fontSize: 13, cursor: 'pointer', background: amount === q.toLocaleString() ? 'var(--accent)' : 'var(--bg-secondary)', color: amount === q.toLocaleString() ? 'white' : 'var(--text-hint)', border: 'none', fontWeight: 600 }}>
            {formatAmount(q)}
          </button>
        ))}
      </div>
      <input className="input" placeholder="직접 입력 (최소 1,000원)" value={amount}
        onChange={e => setAmount(e.target.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, ','))}
        style={{ marginBottom: 10 }} />
      <div style={{ fontSize: 12, color: 'var(--text-hint)', marginBottom: 12, padding: '8px 12px', background: 'var(--bg-secondary)', borderRadius: 8, lineHeight: 1.6 }}>
        💡 오픈채팅방에서 카카오페이로 송금하는 방법이 궁금하다면?{' '}
        <a href={KAKAOPAY_GUIDE} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', fontWeight: 600 }}>카카오페이 송금 가이드 →</a>
      </div>
    ...
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      ...
      <div className="page" style={{ paddingTop: 12 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', marginBottom: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            <div onClick={() => navigate(`/creator/${mission.creator_id}`)} style={{ fontSize: 12, color: 'var(--accent)', cursor: 'pointer' }}>
              {mission.creator?.nickname} ›
            </div>
            <h1 style={{ fontSize: 20, fontWeight: 800, lineHeight: 1.3 }}>{mission.title}</h1>
          </div>
          <span className={`badge ${STATUS_BADGE[mission.status]}`}>{STATUS_TEXT[mission.status]}</span>
        </div>
        ...
        {mission.openchat_link && (
          <div style={{ background: '#FEE50011', border: '1px solid #FEE50033', borderRadius: 'var(--radius-sm)', padding: '12px 14px', marginBottom: 12 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-hint)', lineHeight: 1.6, marginBottom: 8 }}>
              후원 의향 등록 후 알림으로 오픈채팅방 링크를 보내드려요. 오픈채팅방에서 카카오페이로 실제 송금해주세요.
            </p>
            <a href={KAKAOPAY_GUIDE} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: 'var(--accent)', fontWeight: 600 }}>
              카카오페이 오픈채팅 송금 가이드 →
            </a>
          </div>
        )}
        ...
        {myPledge && myPledge.status !== 'cancelled' && (
          <div style={{ background: '#8b5cf611', border: '1px solid #8b5cf633', borderRadius: 'var(--radius-sm)', padding: '12px 14px', marginBottom: 12, fontSize: 13 }}>
        ...
        {mission.status !== 'expired' && !isCreator && !myPledge && (
          <PledgeForm missionId={id} refetch={refetch} />
        )}

        {done && (
          <div style={{ background: 'linear-gradient(135deg, var(--accent)22, #ec489922)', border: '1px solid var(--accent)', borderRadius: 'var(--radius)', padding: 16, textAlign: 'center', marginBottom: 12 }}>
            <div style={{ fontSize: 32 }}>🎉</div>
            <div style={{ fontWeight: 800, fontSize: 17, marginTop: 4 }}>목표 달성!</div>
            <div style={{ fontSize: 13, color: 'var(--text-hint)', marginTop: 4 }}>초과 후원도 계속 받을 수 있어요</div>
          </div>
        )}
        ...
        {isCreator && mission.status !== 'expired' && (
          <>
            ...
            <button className="btn-secondary" style={{ borderColor: 'var(--danger)', color: 'var(--danger)', marginBottom: 12 }} onClick={handleExpire} disabled={expireLoading}>
        ...
        <div style={{ position: 'relative', marginBottom: 16 }}>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn-secondary" onClick={() => setShareMenuOpen(o => !o)} style={{ flex: 1 }}>🔗 공유하기</button>
            <button className="btn-secondary" onClick={() => navigate(`/creator/${mission.creator_id}`)} style={{ flex: 1 }}>👤 프로필 보기</button>
          </div>
          {shareMenuOpen && (
            <>
              <div onClick={() => setShareMenuOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 10 }} />
              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', left: 0, right: 0, background: 'var(--bg-card)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 'var(--radius-sm)', overflow: 'hidden', zIndex: 20, boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
                {[
                  { icon: '🎯', title: '미션 링크 복사', desc: '이 미션 바로 열기', fn: handleShareMission },
                  { icon: '👤', title: '크리에이터 프로필 링크', desc: '모든 미션 보기', fn: handleShareProfile }
                ].map(item => (
                  <button key={item.title} onClick={item.fn} style={{ width: '100%', background: 'none', border: 'none', padding: '14px 16px', textAlign: 'left', color: 'var(--text)', fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        {isCreator && pendingPledges.length > 0 && (
          <div className="card">
            ...
            {pendingPledges.map(p => (
              <div key={p.id} style={{ display: 'flex', gap: 10, alignItems: 'center', paddingBottom: 12, marginBottom: 12, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button onClick={() => handleConfirmPledge(p.id)} style={{ background: 'var(--success)', color: 'white', border: 'none', borderRadius: 8, padding: '6px 10px', fontSize: 12, cursor: 'pointer' }}>✓ 확인</button>
                  <button onClick={() => handleCancelPledge(p.id)} style={{ background: 'none', color: 'var(--danger)', border: '1px solid var(--danger)', borderRadius: 8, padding: '6px 10px', fontSize: 12, cursor: 'pointer' }}>✕</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {confirmedPledges.length > 0 && (
          <div className="card">
            ...
            {confirmedPledges.map(p => (
              <div key={p.id} style={{ display: 'flex', gap: 10, alignItems: 'center', paddingBottom: 10, marginBottom: 10, borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <Avatar user={p.user} size={32} />
